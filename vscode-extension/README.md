...
**What's new in 2.5.0** — **One-click flow building, multi-line labels, and a real colour palette.** Hover any flowchart node to get a small green **"+"** button that opens a 4-shape picker (Process box / Rounded / Circle / Decision); pick a shape and a new connected node is appended in one click. **Multi-line labels everywhere** — the edit modal's name / label field is now a textarea (Enter inserts a line break, Cmd/Ctrl+Enter submits), and real newlines round-trip through the Mermaid source as the standard `\n` escape across node labels, edge labels, and subgraph titles. The **fill and border color** fields now show a curated default palette plus a **Recently used** grid that's seeded from the chart's existing `style` / `linkStyle` / `classDef` colours every time the modal opens. See [CHANGELOG.md](https://github.com/NextGenPowerToys/mermaid-visual-editor/blob/main/CHANGELOG.md).
...
1. Install the extension from the marketplace, or `code --install-extension mermaid-visual-editor-2.5.0.vsix`.
